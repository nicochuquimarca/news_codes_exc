...
aarc_oa.check_duplicates_and_missing_values(original_df = n ,new_df = n_df, column_name = 'PersonId', check_missing_values = True)
...
# Get to know on how many columns I need to perform the test
alt_names_num = n['alternative_names_num'].unique()
alt_names_num 
# Tabulate the number of alternative names
df = n.groupby('alternative_names_num').size().reset_index(name='Counts')
total_c  = df['Counts'].sum() # Calculate the total of the 'Counts' column
df['share'] = df['Counts'] / total_c # Add a new column 'share' that shows the share of each count with respect to the total
df = df.sort_values(by='share', ascending=False) # Sort the DataFrame by the 'share' column in descending order
